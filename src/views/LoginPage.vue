...
export default {
    ...
    setup() {
        ...
        const submitForm = async () => {
            console.log(loginLog.value.userPassword)
            try {
                const response = await axios.post("http://localhost:8080/api/user/login", loginLog.value);
                ...
                if (error.response && error.response.status === 400) {
                    // 400 에러 처리
                    console.error("로그인 실패:", error.response.data.message || "잘못된 요청입니다.");
